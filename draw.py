...
# draw the horizontal and vertical lines of the grid to differentiate
# between the shapes.
...
def draw_grid_lines(surface, grid):
    ...
    for i in range(len(grid)):
        pygame.draw.line(surface, (0, 0, 0), (x, y + i * block_size),
                         (x + play_width, y + i * block_size))

    for j in range(len(grid[0])):
        pygame.draw.line(surface, (0, 0, 0), (x + j * block_size,
                         y), (x + j * block_size, y + play_height))
...
def draw_next_shape(shape, surface):
    ...
    for i, r in enumerate(next_shape):
        ...
        for j, col in enumerate(row):
            if col == '0':
                # drawing each grid of the shape using pygame.draw.rect on the
                # side of the board.
                pygame.draw.rect(
                    surface,
                    shape.color,
...
def draw_window(surface, grid, score):
    ...
    surface.blit(heading, (top_left_x + play_width /
                 2 - (heading.get_width() / 2), 5))
    ...
    pygame.draw.rect(surface, (0, 255, 0), (top_left_x,
                     top_left_y, play_width + 1, play_height + 1), 5)
