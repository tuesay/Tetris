...
def valid_space(shape, grid):
    ...
    # convert_shape_format func. sends the position of the shape's coordinates
    # based on the grid matrix.
...
# if y-axis of pos coordinates is less than 1, its outside the screen -
# meaning game lost.
...
def clear_rows(grid, locked):
    ...
    for i in range(len(grid)):
        row = grid[i]
        # for every row, if no grid is black (all grids filled with shapes),
        # increase inc by 1 and remember the index.
        if (0, 0, 0) not in row:
            inc += 1
            idx = i
    ...
    if inc > 0:
        # update the {locked's key} with new coordinates and push the (entire
        # row + inc) - condition: if the index is above last marked idx.
