...
class Grid:
    ...
    def clear_lines(self):
        ...
        self.grid[:num_cleared_lines] = [[0 for _ in range(self.width)] for _ in range(num_cleared_lines)]
    ...
    def valid_move(self, shape, x, y):
        for i, row in enumerate(shape):
            for j, cell in enumerate(row):
                if cell:
                    if x + j < 0 or x + j >= self.width or y + i >= self.height or self.grid[y + i][x + j] != 0:
                        return False
        return True
    ...
    def draw(self, screen, x_offset=0):
        for y, row in enumerate(self.grid):
            for x, cell in enumerate(row):
                ...
                pygame.draw.rect(screen, color, (x * GRID_SIZE + x_offset, y * GRID_SIZE, GRID_SIZE, GRID_SIZE), 0)
